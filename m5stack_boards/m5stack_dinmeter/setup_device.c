...
/* Panel factory for the M5Stack DinMeter.
 *
 ...
 *
 * The ST7789 panel (1.14-inch 135x240 IPS) ships inside esp_lcd, so no extra
 * component and no vendor init table is needed.
 *
 * NOTE: The 135x240 window sits in the controller's 240x320 GRAM at column
 * +52, row +40. dev_display_lcd cannot express a GRAM offset, so the
 * application must call esp_lcd_panel_set_gap(panel, 52, 40) after init; a
 * driver that ignores it renders shifted.
 */
