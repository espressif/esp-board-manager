...
``sub_type: gpio`` uses a single ``gpio`` peripheral to set the power control pin level; ``active_level`` represents the active-high or active-low sense for power-on. ``sub_type: custom`` registers board-specific lifecycle operations, which supports PMICs, IO expanders, multi-rail sequencing, and other board-owned power implementations.
...
See complete fields: :ref:`power-ctrl-gpio-full`.
...
- A ``custom`` controller must be registered under the same name as its ``power_ctrl`` device and provide ``set_power``.
- The framework manages references in the custom controller's ``peripherals`` list.
...
``periph_handle`` points to the GPIO subtype's underlying peripheral handle. ``custom_context`` is the custom subtype's board-owned lifecycle state. The device is generally invoked indirectly by ``esp_board_device_power_ctrl()`` through the ``power_ctrl_device`` reference and does not need to be operated directly.
