...
- ``gen_skip``: Typically used to override board defaults, indicating that the current project does not use the device or peripheral; it is skipped during parsing and generation. Board-directory ``.c`` files are still compiled into ``gen_bmgr_codes``. Factory functions or ``custom`` implementations that include chip-driver headers must be wrapped with ``__has_include``, or the build fails after those ``dependencies:`` are dropped. See :doc:`board-directory`.
