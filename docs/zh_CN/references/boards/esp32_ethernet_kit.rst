...
.. figure:: ../../../_static/boards/esp32_ethernet_kit/esp32-ethernet-kit-v1.2-overview.png
    :align: center
    :scale: 80%
    :alt: ESP32-Ethernet-Kit v1.2 概图
...
ESP32-Ethernet-Kit 是一款来自 `乐鑫 <https://www.espressif.com/zh-hans>`_ 的开发板。
...
.. figure:: ../../../_static/boards/esp32_ethernet_kit/esp32-ethernet-kit-v1.2.jpg
    :align: center
    :scale: 80%
...
.. figure:: ../../../_static/boards/esp32_ethernet_kit/esp32-ethernet-kit-v1.1-block-diagram.png
    :align: center
    :scale: 60%
    :alt: ESP32-Ethernet-Kit 功能框图
    :figclass: align-center
...
.. figure:: ../../../_static/boards/esp32_ethernet_kit/esp32-ethernet-kit-a-v1.2-layout.jpg
    :align: center
    :scale: 80%
    :alt: ESP32-Ethernet-Kit V1.2 - 以太网母板（A 板）布局
    :figclass: align-center
...
.. figure:: ../../../_static/boards/esp32_ethernet_kit/esp32-ethernet-kit-b-v1.0-layout.png
    :align: center
    :scale: 80%
    :alt: ESP32-Ethernet-Kit - PoE 子板（B 板）布局
    :figclass: align-center
...
.. list-table:: 表格2  PoE 子板（B 板）
  ...
  * - 主要组件
    - 基本介绍
  * - A 板连接器
    - 4 个排针（左侧）和排母（右侧），用于将 PoE 子板（B 板）连接至 :ref:`以太网母板（A 板）<get-started-esp32-ethernet-kit-a-v1.2-layout>`。左侧的管脚接受来自 PoE 交换机的电源。右侧的管脚为以太网母板（A 板）提供 5 V 电源。
  * - 外部电源终端
...
.. figure:: ../../../_static/boards/esp32_ethernet_kit/esp32-ethernet-kit-rmii-clk-from-phy.png
    :align: center
    :scale: 80%
    :alt: PHY 侧提供 RMII 时钟
    :figclass: align-center

    PHY 侧提供 RMII 时钟（点击放大）

请注意，系统上电时 RESET_N 旁的下拉电阻会将 PHY 置于复位状态，ESP32 需要通过 GPIO5 将 RESET_N 拉高才能启动 PHY，只有这样才能保证系统的正常上电，否则 ESP32 会存在一定几率进入下载模式。
...
.. figure:: ../../../_static/boards/esp32_ethernet_kit/esp32-ethernet-kit-rmii-clk-to-phy.png
    :align: center
    :scale: 80%
    :alt: ESP32 APLL 内部提供的 RMII 时钟
    :figclass: align-center

    ESP32 APLL 内部提供的 RMII 时钟（点击放大）

要实现此选项，用户需要在板子上移除或添加一些阻容元器件。有关详细信息，请参见 `ESP32-Ethernet-Kit v1.2 以太网母板（A 板）原理图`_，第 2 页，位置 U2。

.. Note::

    请注意，以下情况下只能使用 `PHY 侧提供 RMII 时钟`_ 或者外部时钟源提供 RMII 时钟：

    * 同时使用以太网和 Wi-Fi，则只能只用外部 RMII 时钟，因为 ESP32 APLL 内部提供的 RMII 时钟不稳定。
    * APLL 已经用于其他用途（如 I2S 外设）。
...
根据 **备注** 描述的不同情形，本连接器包含可用做他用的 GPIO。
...
.. csv-table::
    :header: ESP32-WROVER-E,IP101GRI,UART,JTAG,GPIO,备注
    ...
    IO0,REF_CLK,,,,见下方备注 1
    IO4,,,,IO4,
    IO16,,,,IO16 (NC),见下方备注 2
    IO17,,,,IO17 (NC),见下方备注 2
    IO5,Reset_N,,,,见下方备注 1
    IO18,MDIO,,,,
    IO19,TXD[0],,,,
    IO21,TX_EN,,,,
...
在完成开发环境设置和开发板测试后，即可配置并烧录 `ethernet/basic <https://github.com/espressif/esp-idf/tree/af25eb44/examples/ethernet/basic>`_ 示例。本示例专门用于测试以太网功能，支持不同 PHY，包括 :ref:`ESP32-Ethernet-Kit v1.2 开发板 <get-started-esp32-ethernet-kit-v1.2>` 使用的 **IP101GRI**\ 。
...
* `user_guide_v1.0 <https://docs.espressif.com/projects/esp-dev-kits/zh_CN/latest/esp32/esp32-ethernet-kit/user_guide_v1.0.html>`__
* `user_guide_v1.1 <https://docs.espressif.com/projects/esp-dev-kits/zh_CN/latest/esp32/esp32-ethernet-kit/user_guide_v1.1.html>`__
...
* `JTAG 调试 <https://docs.espressif.com/projects/esp-idf/zh_CN/stable/esp32/api-guides/jtag-debugging/index.html>`_
